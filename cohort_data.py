...
def unique_houses(filename):
    ...
    with open(filename, 'r') as f:
        houses = set([line.strip().split('|')[2] for line in f.readlines() \
...
def find_cohort_by_student_name(student_list):
    ...
    """
    name_to_search_for = input("Who are you looking for? ")
    for student in student_list:
        if student[0] == name_to_search_for:
            return f"{student[0]} was in the {student[-1]} cohort."
...
def find_house_members_by_student_name(student_list):
    ...
    for student in student_list:
        if student[1] == house_name and student[-1] == cohort_year and student[0] != search_name:
            print(student[0])
